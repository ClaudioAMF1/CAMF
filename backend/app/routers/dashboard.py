...
def alertas(dias: int = Query(default=30, ge=1, le=365), db: Session = Depends(get_db)):
    """Boletos abertos vencendo nos próximos N dias."""
    boletos = svc.alertas_vencimento(db, dias)
    saida = []
    for b in boletos:
        out = schemas.BoletoOut.model_validate(b)
        out.pagador_nome = b.pagador.nome if b.pagador else None
        saida.append(out)
    return saida
